...
const { upsertImage, loadImagesMap, markDeleted, setMeta } = await import('../server/utils/db.ts')
...
let inserted = 0, updated = 0, skipped = 0, thumbGen = 0
...
for (const abs of diskFiles) {
  ...
  const changed = !prev || prev.mtime !== Math.floor(stat.mtimeMs) || prev.size !== stat.size
    ...
    try {
      ...
      if (idForName) {
        ...
        if (!fs.existsSync(thumbPath)) {
          try {
            await sharp(abs).resize(320).jpeg({ quality:80 }).toFile(thumbPath)
            thumbGen++
          } catch (e) {
